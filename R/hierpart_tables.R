...
#' models.
#'
#' @param model.names (character) vector of model names located in one or more
#' directories.
...
#' Default = 100.
#' @param set_seed (numeric) initial seed to be set before running analysis.
...
#'                 sample.size = 100, out.dir, verbose = TRUE)
#'
#' @importFrom raster stack
#' @importFrom stats na.omit
#' @export

hierpart_tables <- function(model.names, sp.name, format = "ascii",
                            replicate.numbers, parameters, clim.models,
                            emi.scenarios, iterations = 100, sample.size = 100,
                            set_seed = 1, out.dir, verbose = TRUE) {
    ...
    if (!missing(replicate.numbers) & !missing(parameters)) {
      if (missing(clim.models) & missing(emi.scenarios)) {
        ...
        for (i in 1:length(parameters)) {
          ...
          vals <- get_rvals(model.names, pattern, sample.size, seeds[h])
      ...
      if (!missing(clim.models) & missing(emi.scenarios)) {
        ...
        for (i in 1:length(source)) {
          ...
          for (j in 1:length(parameters)) {
            ...
            vals <- get_rvals(geplic, pattern, sample.size, seeds[h])
      ...
      if (!missing(clim.models) & !missing(emi.scenarios)) {
        ...
        for (i in 1:length(emi.scenarios)) {
          ...
          for (j in 1:length(clim.models)) {
            ...
            vals <- get_rvals(eeplic, pattern, sample.size, seeds[h])
...
get_rvals <- function(model.names, pattern, sample.size = 100, set_seed = 1) {
  gepl <- gregexpr(pattern, model.names)
  gepli <- regmatches(model.names, gepl)
  geplic <- unlist(gepli)
