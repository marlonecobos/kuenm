...
\t  more than one of them where used. Name: binary_comparison.\n\n")
    cat("Description of values in rastes resulted from  comparisons:\n")
    cat("\t- Continuous comparison generally ranges from -1 to 1. Although the range may vary, negative
\t  values represent decrease in sitability and positive values represent increase of this variable;
...
with all sampled data per each iteration can be found. A barplot summarizing the
...
in each HP_results folder.\n\n")
    ...
    cat("\t- hierpart_Goodness_fit.csv contains values that represent how well the sattistical
\t  model fitted the data.\n")
    cat("\t- hierpart_Raw_effects.csv contains values of Independent, Joint, and Total effects
\t  for all interations of the hierarchical partitioning analysis.\n")
...
\t  iterations, expressed as percentages. These values are used to create the barplot.\n")
    sink()
  }
...
of strcit extrapolative areas among distinct GCMs, per each RCP (emission scenario).\n\n")
    cat("Inside each set folder raster files resulted from exploring the agremment of strict
...
used to perform medel projection. The table below indicates what each value in raster
